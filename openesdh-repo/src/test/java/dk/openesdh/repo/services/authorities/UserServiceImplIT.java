...
public class UserServiceImplIT {
    ...
    private static final String CSV_PERSON_USERNAME = "csvtestuser";
    ...
    public void tearDown() {
        if (personService.personExists(CSV_PERSON_USERNAME)) {
            personService.deletePerson(CSV_PERSON_USERNAME);
        }
    ...
    public void shouldCreateUsersFromCSV() throws Exception {
        ...
        Assert.assertTrue("Should create a user from csv", personService.personExists(CSV_PERSON_USERNAME));

        Set<String> hrUsers = authorityService.getContainedAuthorities(AuthorityType.USER, HR_GROUP_NAME, true);
        Assert.assertTrue("HR group should contain uploaded csv user", hrUsers.contains(CSV_PERSON_USERNAME));

        Set<String> itUsers = authorityService.getContainedAuthorities(AuthorityType.USER, IT_GROUP_NAME, true);
        Assert.assertTrue("IT group should contain uploaded csv user", itUsers.contains(CSV_PERSON_USERNAME));
    }
}
