 ...
 */
function getItem(siteId, containerId, pathParts, node, populate, isCase)
{
...
function splitQNamePath(node, rootNodeDisplayPath, rootNodeQNamePath, qnameOnly)
{
    ...
    if (path.match("^"+SITES_SPACE_QNAME_PATH) == SITES_SPACE_QNAME_PATH)
    {
        ...
        //logger.log("\n\n\t\t\t===> search.lib.js - 582 decodedCasePath: "+decodedCasePath+" <===\n\n");
        siteId = decodedCasePath.match(/([0-9]{8})-([0-9]*)/g);

        //logger.log("\n\n\t\t\t===> search.lib.js 585: qpathUnderCasesFolder - "+qpathUnderCasesFolder+" <===\n\n");
        ...
        if (postContainerPosition !== -1){
            ...
            var positionUnderContainer = qpathContainer.indexOf("/");
            //logger.log("\n\n\t\t\t===> search.lib.js - 592  qpathContainer: "+qpathContainer+" <===\n\n");
            if (positionUnderContainer !== -1){
                // extract container id from the qname path - strip off namespace
                containerId = qpathContainer.substring(0, positionUnderContainer);
                //logger.log("\n\n\t\t\t===> search.lib.js - 596  containerId 1st treatment: "+containerId+" <===\n\n");

                containerId = containerId.substring(containerId.indexOf(":") + 1);
                //logger.log("\n\n\t\t\t===> search.lib.js - 599  containerId 2nd treatment: "+containerId+" <===\n\n");
    ...
    return [siteId, containerId, displayPath, isCase];
}
...
function processResults(nodes, maxPageResults, startIndex, rootNode, meta)
{
    ...
    for (var i = 0, j = nodes.length; i < j; i++)
    {
        ...
        try
        {
            ...
            logger.log("\n\t\t\t => (processResults) This is a ("+ parts[0].toString()+")case.\n");
            item = getItem(parts[0], parts[1], parts[2], nodes[i], populate, parts[3]);
            if (item !== null)
            {
                processed++;
                if (populate)
 ...
 * Created by lanre on 05/08/2015.
 */
