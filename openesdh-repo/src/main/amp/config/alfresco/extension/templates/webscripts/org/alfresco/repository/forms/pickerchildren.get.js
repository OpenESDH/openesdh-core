...
function main() {
    ...
    try {
        ...
        in
            ...
            else if (argsSelectableType == "contact:person") {
...
//Find contact types
function findContacts(searchTerm, type, results){
    logger.warn("--> Calling contactUtil\n\n");
    var personRefs = contactUtil.getContactsById(searchTerm, type);

    logger.warn("--> returned from contactUtil\n\n");

    // create person object for each result
    for each(var personRef in personRefs){
        logger.warn("found personRefs = " + personRefs);

        results.push({
            item: createContactResult(search.findNode(personRef)),
            selectable: true
        });
    }
}
...
/**
 * Creates a contact object more like the same object returned from the personObject returned from
 * the imported function, createPersonResult in pickerresults.lib.js
 */
function createContactResult(node)
{
