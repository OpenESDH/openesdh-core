...
<div class="form-field">
   ...
   <#else>
      ...
      <#if field.control.params.options?? && field.control.params.options != "">
        <#assign size=field.control.params.options?split(optionSeparator)?size >
         <select id="${fieldHtmlId}-entry" name="s_${field.name}" multiple="multiple" size="${size}" tabindex="0" onchange="javascript:Alfresco.util.updateMultiSelectListValue('${fieldHtmlId}-entry', '${fieldHtmlId}', <#if field.mandatory>true<#else>false</#if>);"
         ...
         <#if field.control.params.mode?? && isValidMode(field.control.params.mode?upper_case)>
            <input id="${fieldHtmlId}-mode" type="hidden" name="${field.name}-mode" value="${field.control.params.mode?upper_case}" repo="multisearch2" />
         </#if>
      <#else>
